...
def main(): # main
    ...
    for i in range(0, len(grid)):
        for j in range(0, len(grid[i])):
            ...
            # 1
            if (i + 3) < len(grid) and (j + 3) < len(grid[i]):
                product = int(grid[i][j-1]) * int(grid[i+1][j-2]) * int(grid[i+2][j-3]) * int(grid[i+3][j-4])
                if greatest_product < product:
                    greatest_product = product
